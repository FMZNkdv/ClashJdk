...
public class Device {
    ...
    public void processPacket(int packetID, byte[] payload) {
        System.out.println("[S] " + packetID + " received");
        try {
            ...
            if (PacketsFactory.availablePackets.containsKey(packetID)) {
                ...
                System.out.println("[S] " + packetID + " not handled");
            }
        } catch (Exception e) {
            System.out.println("[S] Error while decrypting / handling " + packetID);
            e.printStackTrace();
        }
    }
